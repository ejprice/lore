...
# 01 — Both floors, and the pre-registered acceptance verdicts
...
## `choose_cosine_floor` selection

Each floor was selected by `choose_cosine_floor` against its own labeled sets, with the anchor
gate applied afterwards: the absence verdict fires on
`max_cosine < floor AND NOT has_verbatim_anchor`.

## Pre-registered acceptance

The bars were registered before the run. Both legs are judged on the ORIGINAL labeled sets, with
response-best cosines captured by the PORTABLE instrument.
...
The determinism control's verdict is in `07`.
