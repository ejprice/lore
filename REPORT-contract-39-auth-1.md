...
| TTL positive/negative caches | `cachetools` — **NOT INSTALLED** (`importlib.metadata.version("cachetools")` → `PackageNotFoundError`) | odoo-code's production use (`code_mcp/auth.py`: `TTLCache(maxsize=500, ttl=300)` positive, `TTLCache(maxsize=1000, ttl=60)` negative) — its API surface is what the design's §14 row cites. | **replace** the dict-with-timestamps pattern — **BLOCKED: needs install authorization (§7-B1).** My pins deliberately assert cache BEHAVIOUR (call counts, per-instance isolation, TTL semantics) and name no library, so they neither force nor forbid the specific package. |
