# PKT-26 — Semantic summary reuse (operator-promoted 2026-07-11)
size ~0.25 wu · wave F · depends: PKT-14 loresage (synthesis); PKT-20 aggregates helpful, not blocking
...
- **Match**: cosine against the stored key above a MEASURED threshold (survey-derived
  ...
  machinery from PKT-02 is the template).
- **Invalidation, deterministic**: serve only while EVERY cited hash is live-current;
  one drift → drop + queue regeneration. (Shares the drift machinery with PKT-25.)
...
## Entry check
loresage live (PKT-14); if PKT-20 landed, pull its repeat-query aggregates to pick
