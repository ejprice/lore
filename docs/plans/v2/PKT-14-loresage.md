# PKT-14 — loresage: the LLM-backend package
...
embedder ABC. Pure infrastructure — no worker, no tool changes (PKT-15 consumes it).
...
- Token/model/cost stamping hooks (the trace columns exist since P2) so PKT-15/20 can
  account per call.
...
## Scope OUT
- The enrichment worker, sweep detectors, any serving change (PKT-15/16).
