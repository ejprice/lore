...
| 63 | **Governed-tool retrofit: comms + memory** — add `owner`+`scope` (server-stamped, never a caller arg), route every read through `authorize_filter` and every write/delete through `authorize`; migrate existing free-form-owned rows (dirty-store, #107/#131 class). Design §4/§7 | — | **D** | 0.30 →split if ≥ | 60, 61, 62 | **IN PROGRESS — 63a (substrate+memory) CODE-BUILT but NOT closed.** Substrate + memory retrofit + F5 + #439/#425 built & scoped-cold-audit-GO (HEAD `c794b21`, reports `receipts/2026-09-02-packet63a/`), but the **closing currency gate is RED**: 13 repo-wide invariant tests, **ALL 63a-caused** (finding #452 — the scoped cold-audits never ran the full suite). Fix wave = task `b5909325` (7 reconciliations). ⚠ BLOCKED: subagent spawns stalling at startup this session (3× consecutive) — needs a fresh session to run the fix wave. 63b/63c not started. Root-fixes ledgered: #449 (task `571ef1a`), #436/#437/#441 → 63b. |
