...
| 48 | **principals substrate** — `build_store(config)` extraction FIRST (3-way copy today), then the `principal` table (never `user`) | — | D | 0.20–0.25 | 45 (∥ 46/47) | **✅ DONE 2026-08-20 — COMMIT-ONLY, NO DEPLOY** (49/39 serve it). Opus 4.8 end-to-end + Fable design sidecar; coordination on lore_comms. **48-A `1bbb0fb`**: `build_store(config)` NARROW extraction (un-readied), 3-way copy unified, mutation-proven routing-is-sharing; contract 11 pins → adversary r2 → cold audit GO. **48-B `2c33007`**: `principal` table (Variant A, call-time authz-domain ASSERTs, folded into `generate_ddl`) + PrincipalStore CRUD (new `principals.py`); **Model B** — subject `option<string>` UNIQUE, live-probe-verified multiple-NONE coexist + same-non-NONE rejected incl. on UPDATE; set_subject unconditional (F1b=A); contract 56 pins → adversary r3 (caught a QUANTIFIER authz-domain leak: a `superadmin` role could have leaked) → cold audit GO (64 green, 14/14 mutations, #107 dirty-store fixture, #120 seam gate observes PrincipalStore). Operator: F1=model B, F1b=A, scope=schema+store+CRUD, per-slug now. Design `131ba56`. Findings #388/#390/#391 open, #389 resolved. Reports → `receipts/2026-08-20-packet48/`. |
| 49 | **principal CLI (1B) + per-user API keys (1C)** — 39 §4's mint, never odoo-code's; hashed keys; revocation beats cache | — | D | 0.25 | 48 | **✅ DONE 2026-08-20 — COMMIT-ONLY, NO DEPLOY (operator ruled: batch the deploy with 39/56, where "configure login first, then flip posture" applies).** Opus 4.8 end-to-end + Fable design sidecar; all coordination on lore_comms. Ships: `principal_key` table (hash UNIQUE, name, principal `record<principal>`, created_at, expires_at/revoked_at option<datetime>; UNIQUE(hash)+UNIQUE(principal,name); folded into `generate_ddl`) + `PrincipalKeyStore` (clones the PrincipalStore owner idiom; `verify` re-checks 4 conditions EVERY call — no cache, revocation beats cache R12; uniform deny/no oracle; routes the principal map through the SHARED `_row_to_principal`) + CLI (`lore-adm` console script — operator-ruled invocation, NOT `-m loremaster.principals`: add/list/delete/suspend/unsuspend/set-expiry/**--role** + mint-key/list-keys/revoke-key; NO `--execute`; creds-free config; sanitised list renders) + `PrincipalStore.delete`(cascade, one txn, →int)/`set_expires`. Pipeline: design → contract r1→r2→r3 → **adversary 3 passes** (r1: 3 C-DEF builder-traps + missing list-keys sanitiser pin + harness defect; r2: #353 retry-seam map; r3 SUFFICIENT, ref build 762/0) → build → DRY promotion (`load_surreal_only_config`→config, mutation-proven) → cold audit (NO-GO on ONE prose word `high-entropy token`, else all-green: 29/29 live probes + mutation + contract-blind diff) → fixes. Operator rulings: key `<name>:<secret>` (opaque declined); NO dry-run/`--execute` ("Gate none"); creds-free CLI; `--role {member,admin}` added; harness import-count docstring DELETED (mutable→don't cite). 1046 tests green; currency PASS (191 typecheck/446 pytest RED are the #333 packet-39-owned bound, 0 packet-49). Design `938c9ce`; final `02823b2`. Finding #392 resolved. Reports → `receipts/2026-08-20-packet49/`. |
...
| 39 | hosted-security (design first; REQUIRED before off-LAN) — **PULLED FORWARD to wave D (operator, 2026-08-02): #296 is ANSWERED by packet 45's allowlist; the contract is RE-CUT against the principal substrate (48/49 replace R12's roster file — the standing Log override) and the revision gets a FRESH adversary pass (standing law)** | PKT-21 | **D** (was S) | 0.30 →split | 45, 48, 49, 59 | **DESIGN RULED + CONTRACT WRITTEN; was BLOCKED on #296 — now UNBLOCKED by 45.** Design `docs/design/2026-07-31-packet39-google-oauth.md` (R1–R16, twelve+ rulings). Contract **480 pins / 444 RED / 36 GREEN**, rest of suite **7705 passed / 0 failed**, ruff clean; satisfiability re-discharged 0-failed on every revision. **FOUR adversary passes, all INSUFFICIENT**, each finding a real blocker every prior gate passed — one root cause (`_setup_handlers` binds at construction ⇒ post-construction installs are live in-process, DEAD ON THE WIRE): WB30 `call_tool` → WB48 guard *after* the tool body → WB93 instance attr on `list_tools` → WB100 **class** attr (flaky-green 5/10). **Build never started, deliberately.** #296's answer is scoped in `docs/design/2026-08-01-multi-user-lore-proposal.md` §2. Filed #291 #294 #295 #296. Receipts → `receipts/2026-07-31-packet39/`. Operator-side and untouched: hades SNI route · claude.ai client secret · the posture flip. **#295 STRUCTURAL LEG FOLDED IN HERE (operator, 2026-08-09):** the "a refused tool is not dispatchable — gate REGISTRATION/VISIBILITY, not INVOCATION, so there is no 'after' to place a guard in" answer (this packet's own WB30→WB100 root cause; entangles #296 + the #333 auth WIP) is OWNED BY THIS PACKET. The 2026-08-09 defect-class session builds only #295's auth-independent "observe-the-EFFECT-not-the-exception/marker" pin-invariant, and only if the designer rules it cleanly separable from auth. **⚠ RE-CUT + REFRAMED 2026-08-21 (design-sidecar-39-2 + operator): now READ-ONLY hosted; ALL write (member row-scoped + admin) → the new authorization/Keep track (60–65), which deploys WITH 39 as ONE cutover (owned by packet 65; NO 39-alone rebuild/redeploy). New design `docs/design/2026-08-21-packet39-recut-oauth.md` (superseding banner; stack reconciled onto standalone fastmcp; F4/F7 superseded); authz model `docs/design/2026-08-21-lore-authorization-model.md`. 39 ships auth + read + the forward-compat SEAMS (identity carries principal+agent+role; role→capability a function; the #295 middleware composes with the row-level PDP). Depends on += 60–65 for the joint deploy.** **✅ CODE COMPLETE 2026-08-22 — COMMIT-ONLY (deploy = packet 65 joint cutover).** 3 waves, each contract→adversary(SUFFICIENT after 1 revision)→build→cold-audit(GO)→commit (Opus 4.8 end-to-end + Fable design sidecar; lore_comms coordination; operator offline for the build): W1 identity&admission `d4605ec`/`207578f`/`feea77a` (LoreTokenVerifier — hand-rolled Google authN #393, api-key via PrincipalKeyStore.verify #206, email-keyed admission on the 48/49 substrate, role→capability a function, (principal,agent) seam); W2+3 read-only enforcement + composition + config `cdd8e11`/`62f5316` (one deny-by-default readOnlyHint middleware [#295 EFFECT, coverage-as-checked-variable, composes with the RBAC PDP]; FastMCP(auth=)+build_mcp_server http_client seam; BearerAuthMiddleware/AuthVerifier DELETED; OriginValidationMiddleware outermost; config no-client_secret [ESC-2]; posture derivation). Gates: wave-1 41/41, wave-2/3 90/0, loopback #295 7p/1s, typecheck 0, ruff clean, currency PASS (bounds:[]). Fulfilled #395 + cold-audit R2. Residuals: config.keys/ApiKeyVerifier orphaned (kept-but-unwired, disposition→sidecar); 4 test_auth.py keeper docstrings cite §8 not §9 (minor). Reports → `receipts/2026-08-21-packet39-recut/`. ⚠ NO 39-alone deploy — packet 65 owns the joint 39+60–65 cutover. |
| 60 | **Keep substrate** — the `keep` table (typed: project/team/session/dm) + the `member_of{rank}` household edge + `keeps` ownership; `lore-adm` verbs (create-keep, add/remove household, set-rank). Mirrors the 48/49 principal/principal_key build. Design `docs/design/2026-08-21-lore-authorization-model.md` §4.4 | — | **D** | 0.30 | 48, 49 | **✅ CODE COMPLETE 2026-08-22 — COMMIT-ONLY (deploy = packet 65 joint cutover).** 2 waves + a consumer-law amendment; full pipeline (contract→adversary→build→cold-audit) Opus 4.8 end-to-end + Fable design sidecar; lore_comms coordination; operator offline. **W1 schema+`KeepStore` `efccdc8`** (Forks A–F: keeper an INDEXED `keep.keeper` FIELD LINK — **Fork A DIVERGES from design §2.1's `keeps`-edge notation**, sidecar-ruled within delegated authority [semantically identical; the PDP never traverses `keeps`; store-law §4 scalar→field], **countermand-able on operator return**; `member_of{rank}` OVERWRITE TYPE RELATION ENFORCED+UNIQUE(in,out); typed `keep` table with call-time-derived `type`/`rank` domains; `generate_keep_ddl` folded after principal_key; atomic `create_keep`+auto-household [Fork D]; nameless-keep read-back [adversary attack-9]; `_query` shared-retry). **Q2b consumer-law amend `e019477`** (`KeepStore` wraps raw engine rejections as `KeepStoreError` at every write path — coverage-as-checked-variable, mutation-proven; FR-2 Q2). **W2 `lore-adm` verbs `bd9032f`** (create-keep [prints id; per-type `--name` reject, FR-2 Q1] / add-household / remove-household / set-rank; `_dispatch_keep` catches `KeepStoreError`, FR-2 D1) **+ FR-3 `db719a0`** (remove-household LOUD on a ghost keep — access-control; the ∀-verb nonexistent-`--keep`-is-loud invariant, derived from `build_parser`). Gates: 800 pytest, typecheck 0, ruff clean; cold-audit GO (30-check live probe with positive controls). Design forks ruled by the Fable sidecar (Forks A–F + FR-1/2/3, `docs/design/2026-08-22-packet60-keep-substrate-rulings.md`). Findings: #397 RESOLVED (retry-seam registration, #353 recurrence); **#398 OPEN** (stale-STUB→greened-code invariant = Part 3, a standalone hygiene item ruled by the sidecar, triggered **before packet 61**), #400 OPEN (DRY: extract a `lorerunes` `wrap_engine_rejection` shared by Principal/Keep stores), #401 OPEN (test-env-fiction: `lore-adm` store-rejection stderr). Reports → `receipts/2026-08-22-packet60/`. ⚠ NO 60-alone deploy — packet 65 owns the joint 39+60–65 cutover (§8). |
| 61 | **Authorization PDP + audit** → **SPLIT 61a/61b** — the in-process `authorize`/`authorize_filter` engine + the append-only ADMIN-EXEMPT `audit` store. Design §5/§6/§9 | — | **D** | 0.30 →split | 60 | **DESIGN RULED + SPLIT (operator-ratified 2026-08-23).** Sidecar doc `docs/design/2026-08-22-packet61-pdp-audit-rulings.md` (Forks A–L + FR-4). Operator ratified all flagged choices: single-brain = a closed predicate **IR** with two total interpreters (`to_surql`+`matches`), `authorize()==authorize_filter().matches()`, equivalence pinned by a **LIVE-STORE oracle** (not a mock); owner = **two indexed fields** `owner_principal`+`owner_agent`; admin short-circuit = a node OF `authorize_filter`; audit fires on **load-bearing admin bypass**; principal-private write = **exact (principal,agent)**; keeper-on-principal-delete = **refuse-while-keeping** (#402/§FR-4). **✅ CODE COMPLETE 2026-08-23 — both 61a + 61b shipped, COMMIT-ONLY (deploy = pkt 65 joint cutover). See 61a/61b rows.** |
| 61a | **Foundations + Audit** — **w1** #402 keeper-on-principal-delete refuse-while-keeping + admin reassign-keeper/delete-keep verbs + `member_of` auto-cascade live probe + exact-set pin update + no-dangle pin (clears the RED_ORPHANED gate) · **w2** `lorerunes.wrap_engine_rejection` extraction (#400, 3rd consumer) · **w3** #398/#399 recurrence invariant (structural fold-coverage pin) · **w4** append-only ADMIN-EXEMPT `audit` store. Ruling §G/I/J/FR-4 | — | **D** | ~0.20–0.25 | 60 | **✅ CODE COMPLETE 2026-08-23 — COMMIT-ONLY (deploy = pkt 65 joint cutover).** All 4 sub-waves shipped, each full pipeline (contract→adversary→build→cold-audit GO→my currency verify) Opus 4.8 end-to-end + Fable sidecar. **w1 #402** `66b795c` (refuse-while-keeping + set-keeper/delete-keep; member_of auto-cascade probe-settled; RED_ORPHANED→GREEN). **w2 #400** `6ad460b` (two-layer wrap seam `lorerunes.reclassify`+`_txn.wrap_store_rejection`, 9 clones routed; property-derived spelling-agnostic shape-guard). **w3 #398/#399** `708c74e` (structural fold-coverage invariant, non-vacuous 28/28, now live-gating). **w4 audit store** `4f36782` (append-only MRO-clean surface, ulid, born-wrapped, composable fragment; §9 identity-at-write actor_email/actor_agent_name survives-delete; dangle-tolerated cascade classified). Findings: #398/#399/#400/#402/#403 RESOLVED; #404–#412 ledgered (bounds/deferrals/flakes). Reports → `receipts/2026-08-23-packet61/`. ⚠ NO deploy — packet 65 joint cutover. **61b NEXT** (the PDP core). |
| 61b | **PDP engine + resolver + coverage** — **probe** (owner-field IN-index/composite/OR-planner + resolver leading-col, live 3.2.4) → **w1** PDP core in `lorerunes` (IR + `to_surql` + `matches` + `Action` enum + `Subject`/`Resource` + per-action predicates + admin node + audit-obligation flag + the LIVE-STORE oracle equivalence pin) → **w2** visible-Keeps resolver (`KeepStore.list_keeps_for_member`) + coverage-as-checked-variable → **dedicated SECURITY-AUDITOR**. Ruling §A–H | — | **D** | ~0.25–0.30 | 61a | **✅ CODE COMPLETE 2026-08-23 — COMMIT-ONLY (deploy = pkt 65 joint cutover).** Probe + 2 waves, full pipeline (contract→adversary SUFFICIENT→build→cold-audit GO→lead currency-verify) Opus 4.8 end-to-end + Fable sidecar. **probe** `c6c6bc2` (owner-field IN→UnionIndexScan, composite leading-col-only, **IN-inside-OR is a TableScan trap → expand to per-value equalities**; store ref §2 folded). **w1 PDP core** `3b0894e` — one authorization brain in `lorerunes.pdp`: a closed predicate **IR** (ScopeEq/OwnerPrincipalEq/OwnerAgentEq/ScopeInKeeps/And/Or/AllRows/NoRows) with two TOTAL interpreters (`to_surql`+`matches`), `authorize()==authorize_filter().matches()` **true by construction**, pinned by a **LIVE-STORE 3.2.4 differential oracle** (never a mock); admin short-circuit a node OF the filter; audit-obligation flag; frozen `Subject`/`Resource`/`Decision`. **Dedicated SECURITY-AUDITOR** caught **#416** (MEDIUM: `ScopeInKeeps.to_surql` emitted unparenthesized disjuncts — safe at top-level Or but a cross-principal leak under And-composition; the entry-point oracle couldn't see it) → every node now emits a self-contained parenthesized fragment + a live composition-safety pin (mutation M7) + SEC-F2/F3/F4/R4 fixes. **w2 resolver+coverage** `af04b4a`/`650b37f`/`252c221` — `KeepStore.list_keeps_for_member` (plain-table leading-col IndexScan, bare ids, whole-method born-wrapped — it feeds the authz path) + `resolve_visible_keeps` (bare ids → `keep:<id>` frozenset via the SHARED `keep_scope`, the ONE spelling) + `partition_tools_by_population` (Fork H-a, DEFAULT-GOVERNED fail-closed, derived over the live registry, reds-until-reviewed growth pin; shared_read seed = 9 per R1). Gates: 28 pins, full suite 8798p/0f solo, typecheck 0, ruff clean, currency PASS, all mutation proofs RED-and-restored. Sidecar rulings Forks A–L + FR-4 + Fork F/H addenda + R1 (`docs/design/2026-08-22-packet61-pdp-audit-rulings.md`). Findings #413 (IN-in-OR) + #416 RESOLVED; #404–#419 ledgered (bounds/deferrals: #417 reach-law STOP, Fork H-c verb-routing→pkt 63, #406 read-gap fold). Reports → `receipts/2026-08-23-packet61b-w1/` + `receipts/2026-08-23-packet61b-w2/`. ⚠ NO deploy — packet 65 joint cutover. **PACKET 61 COMPLETE.** |
| 62 | **Agent-identity binding (anti-spoofing)** — server-bound, short-lived `(principal, agent)` credentials + the two anti-injection invariants (identity/owner NEVER from a tool argument; scope-on-write PDP-validated). The §3.3/§10-O seam; dedicated SECURITY-AUDITOR pass (it IS the anti-spoofing surface). Design §3 | — | **D** | 0.25–0.30 | 39, 60 | **✅ CODE COMPLETE 2026-08-24 — COMMIT-ONLY (deploy = pkt 65 joint cutover).** 2 waves + a resolved design fork, full pipeline (Fable design sidecar → contract → adversary → build → cold-audit → **dedicated SECURITY-AUDITOR**) Opus 4.8 end-to-end; lore_comms coordination; operator ruled the mint-mechanism forks. **Design** `29e15f2`/`de781b7`/`6d6c63c`/`e34cf47` (`docs/design/2026-08-24-packet62-agent-identity-rulings.md`): mechanism **(A)** app-layer per-agent CAPABILITY (operator-ruled — **no MCP client presents a per-subagent transport credential, HIGH-verified** by a claude-code-guide pass; per-agent identity MUST be app-layer); **real local principal via api-key** (operator OVERRODE the sentinel); scalar `agent.owner_principal` field-link (Fork 3); anti-injection as a DERIVED reach pin (Fork 4). **W1 store foundation** `3c94ffd`: `agent.owner_principal : option<record<principal>>` owns-edge + non-unique index; delete DANGLE-tolerated (#423 docstring — `owner_principal` is the **FOURTH** `record<principal>` link, the missed third being `audit.actor_principal`); contract 50/0, cold-audit GO, live 3.2.4 dirty-store migration. **W2 identity mechanism** `6b180f0`→`9a18c34`→`8408e1b` (contract, **2 converging adversary rounds** — mechanism strong, 18 wrong-builds caught; C-DEF + composite-index + ownerless-deny closed) → `66b5fa4` (build): `agent.capability_hash` UNIQUE + mint-in-register (a **VERIFIED credential**, not a claim — SF-1 ruled) + `verify_capability` (**THE BINDING** `owner_principal==token principal`, fail-closed on a None owner) + `stamp_owner` in **loremaster** (ESC-1) + `resolve_agent` fail-closed None=DENY + `lorerunes.parse_credential` DRY (shared w/ PrincipalKeyStore, mutation-proven, 0-regression) + `agent_of` stays None; lifecycle-scoped capability (SF-2, enforced-when-set expiry seam ESC-3). Cold-audit GO + **dedicated security-auditor GO (36 live exploits, 0 succeeded; §9 anti-spoofing bar CLEARED; #105 dangling-owner surface closed; PDP byte-unchanged)**. Gates: contract 64/0, blast 1388/0 (PrincipalKeyStore 0-regression), typecheck 0, ruff clean, **currency PASS**. Findings #422 (ws resets, open), #423 (RESOLVED), #424 (dnd-probe multi-gate contamination → deleted `d860b16`, RESOLVED), #425 (63/64 `stamp_owner` TOCTOU forward note). Reports → `receipts/2026-08-24-packet62/`. ⚠ NO deploy — packet 65 joint cutover; fleet-must-present-the-capability-every-call is a **pkt-65 orchestration** obligation, not a 62 build. |
| 63 | **Governed-tool retrofit: comms + memory** — add `owner`+`scope` (server-stamped, never a caller arg), route every read through `authorize_filter` and every write/delete through `authorize`; migrate existing free-form-owned rows (dirty-store, #107/#131 class). Design §4/§7 | — | **D** | 0.30 →split if ≥ | 60, 61, 62 | **✅ 63a CLOSED 2026-09-03 — COMMIT-ONLY (deploy = pkt 65 joint cutover).** Substrate + memory retrofit + F5 + #439/#425 built & scoped-cold-audit-GO at `c794b21` (`receipts/2026-09-02-packet63a/`); the closing currency gate then caught **13 repo-wide invariant tests RED_ORPHANED, ALL 63a-caused** (finding #452 — the scoped cold-audits never ran the full suite). **Fix wave (task `b5909325`, 7 reconciliations — each a NEW 63a element registered into a STANDING derived invariant, NEVER hack-to-green; RED-before/GREEN-after each):** fix2 `f4a00fb` (rebuild gate → SUBSTANTIVE emptiness, not raw list len — 63a-ii's withheld notice had defeated it), fix1 `0c5fc23` (`capability=` crossref → `_guarded_description`, byte-exact at full universe), fix3 `278f524` (5 evidence-backed whole-tree-clone allowlist entries — the flagged offenders were 5 test files, NOT `_governed_contract`'s F5 parser which the `glob("test_*.py")` detector can't see), fix4 `5086235` (`capability` param → `_NOT_SERVED_OUT`; credential value rendered at NO served site), fix5 `0ed3215` (`get_or_create_keyed` → KeepStore fault-injection map), fix6 `2f2caae` (2 63a spike-probe `SecretStr("spikeroot")` origins adjudicated), fix7 `bb31a86` (fold-coverage → ROOTED transitive closure + 2 discrimination pins; production schema UNCHANGED). **Currency gate PASS (0 RED_ORPHANED, true exit 0), ruff + typecheck GREEN — TWO independent full-repo runs.** ⚠ Both fresh-context cold-auditor spawns STALLED (prior session's subagent-stall issue reproduced) → LEAD SELF-AUDIT with EMPIRICAL mutation proofs (fix2 revert→A8a RED; fix7 unroot→discrimination pin RED; fix1 byte-exact vs git) — GO, no suppression/weakening/false-clear (`receipts/2026-09-03-packet63a/REPORT-coldaudit-63a-lead.md`). Fix reports → `receipts/2026-09-03-packet63a/`. 63b/63c NOT started. Root-fixes ledgered: #449 (task `571ef1a`), #436/#437/#441 → 63b. |
| 64 | **Governed-tool retrofit: tasks + findings** — the same owner+scope + PDP routing + dirty-store migration for the two ledgers; a Keep task is claimable, a Keep finding resolvable/supersedable, by any household member. Design §4/§7 | — | **D** | 0.30 →split if ≥ | 60, 61, 62 | **PLANNED.** |
| 65 | **Hosted-write enablement + JOINT deploy** — flip the governed tools to hosted-writable via the PDP+Keep model; owns the ONE 39+Keep cutover (rebuild+recreate, posture flip, edge/connector) that ALSO lands the commit-only 49/56 batch. Dirty-store migration + in-image conformance (#107/#131/#139); NO 39-alone deploy (RULED §8). Design §8 | — | **D** | 0.25 | 39, 60, 61, 62, 63, 64 | **PLANNED — owns the single joint deploy** (auth + read + row-scoped write together; no useless read-only-hosted interim). ⚠ **PRE-DEPLOY BLOCKER (finding #396, packet-65-gating):** a stray `config.auth.keys` must MIGRATION-REJECT loudly (or be migrated into `principal_key`) BEFORE the new LAN wire deploys — packet 39 wave-2/3 moved LAN auth off `config.keys`, but the backward-compat pin checks PARSING not AUTHENTICATION, so a `keys`-carrying deploy would silently authenticate nothing (#131 silent-swallow). Same cleanup wave: DELETE the orphaned `ApiKeyVerifier`/`build_api_key_verifier` (gated on adjudicating constant-time vs `PrincipalKeyStore.verify`). |
...
- 2026-08-20 · **PACKET 48 (PRINCIPALS SUBSTRATE) DONE — commit-only, NO DEPLOY (49/39 serve it);
  Opus 4.8 end-to-end + Fable design sidecar; all coordination on lore_comms.** 48-A `1bbb0fb`:
  `build_store(config)` NARROW extraction (un-readied) unifying the 3-way store-construction copy,
  mutation-proven routing-is-sharing (change the factory → all 3 callers redden). 48-B `2c33007`: the
  `principal` table (Variant A, folded into `generate_ddl`, closed authz-domains derived at CALL TIME —
  mutation-provable) + PrincipalStore CRUD (new `principals.py`, clones the FindingLedger owner idiom,
  routes `_query` through the shared seam observed by the #120 gate); **Model B** — subject
  `option<string>` UNIQUE, live-probe-verified (multiple NONE coexist for email-pre-create; same
  non-NONE rejected on CREATE **and** the fill-UPDATE); set_subject unconditional (F1b=A). Both waves
  contract → adversary → build → cold-audit GO; the 48-B adversary caught a QUANTIFIER authz-domain
...
- 2026-08-20 · **PACKET 49 DONE — COMMIT-ONLY, NO DEPLOY (operator: batch with 39/56).** Full
  contract-first cycle: design (Fable) → contract r1→r2→r3 → adversary ×3 (r1 3 C-DEF builder-traps
  + missing list-keys sanitiser + harness defect; r2 #353 retry-seam map; r3 SUFFICIENT 762/0) →
  build → DRY promotion (`load_surreal_only_config`→config, mutation-proven) → cold audit (NO-GO on
  ONE prose word, else all-green) → fixes. Ships `principal_key` + `PrincipalKeyStore.verify`
  (no-cache/uniform-deny/4-recheck) + the CLI (+`--role`) + `delete`/`set_expires`. Operator
  rulings: `<name>:<secret>`, no dry-run/`--execute`, creds-free, `--role`, harness-count deleted.
  1046 green; currency PASS. Final `02823b2`; reports `receipts/2026-08-20-packet49/`; #392 resolved.
- 2026-08-20 · **PACKET 49 follow-up — `lore-adm` console script (operator correction).** The admin
  CLI invocation is `lore-adm`, NOT `python -m loremaster.principals` (works but a poor interface).
  Added `[project.scripts] lore-adm = "loremaster.principals:main"` to loremaster/pyproject.toml,
  prog → `lore-adm`, + a resolution pin (tomllib-reads pyproject, asserts the entry-point target is a
  real callable). `.venv/bin/lore-adm --help` runs; 45 CLI tests green; NO uv.lock delta; no new
  registration site. Commit-only — materializes in `.venv/bin/` at the batched 39/56 image rebuild.
- 2026-08-21 · **PACKET 39 RE-CUT + REFRAMED to READ-ONLY; the AUTHORIZATION / KEEP track (60–65) minted.**
  A design-sidecar session (Fable, `design-sidecar-39-2`) with the operator across a live design
  conversation. (1) **Packet 39** — the stale 2026-07-31 design was re-cut onto standalone fastmcp
  (post-59) and REFRAMED to **read-only hosted**; new design `docs/design/2026-08-21-packet39-recut-oauth.md`
  (superseding banner; F1 email_verified precondition, F4 role-gated-write ruled *then SUPERSEDED*).
  ALL hosted write deferred to the Keep track. (2) **A foundational AUTHORIZATION MODEL was designed
  and RULED:** `docs/design/2026-08-21-lore-authorization-model.md` — per-row `owner`+`scope`, an
  IN-PROCESS PDP over the SurrealDB graph (mirroring Permit.io's verified SurrealDB architecture;
  native store `PERMISSIONS` is INERT under root — a #107-class trap; pycasbin rejected for
  split-brain; framework survey verified by a research scout), the **single-brain-per-action**
  invariant (`authorize(row) ≡ authorize_filter().matches(row)`), the **Keep** collaboration
  primitive (RULED: owner=**keeper** / members=**household** / per-Keep role=**rank** seam, flat now;
  typed project/team/session/dm; a DM = a 2-member Keep), the **3-way write rule** (private=owner ·
  keep=household collaboration · server=owner+admin; hard-delete=owner+admin), **admin = full,
  unrestricted superuser** with an append-only ADMIN-EXEMPT `audit` store, and the anti-spoofing /
  anti-injection identity design (server-bound `(principal,agent)` + identity-never-from-arguments +
  scope-on-write-PDP-validated). (3) **INDEX: packets 60–65 minted** (Keep substrate · PDP+audit ·
  agent-identity binding · governed-tool retrofit comms+memory / tasks+findings · hosted-write
  enablement + JOINT deploy), positioned right after 39 in wave D, sized 0.25–0.30 each; **39 + 60–65
  ship as ONE cutover** (65 owns it; NO 39-alone redeploy; the batch also lands commit-only 49/56).
  ⚠ NUMBERING: 60–65 follow the packet-42/59 number-past-tail precedent (table row position = order
  of record); if the operator prefers `39a–f` suffixes to signal the 39-coupling, it is a trivial
  renumber. NOT YET CONTRACTED — each runs the full design→contract→adversary→build→cold-audit→
  **security-auditor** pipeline. Design docs + this INDEX edit UNCOMMITTED at time of entry (operator
  to say the word).

- 2026-08-22 · **PACKET 39 (hosted-security OAuth, RE-CUT READ-ONLY) — CODE COMPLETE, COMMIT-ONLY.**
  All 3 waves committed on `feat/surreal-unification` (Opus 4.8 end-to-end + Fable design sidecar;
  all coordination on lore_comms; operator offline for the build, design forks ruled by the sidecar).
  W1 identity&admission `d4605ec`→`207578f`→`feea77a`; W2+3 read-only enforcement + composition +
  config `cdd8e11`→`62f5316`. Ships LoreTokenVerifier (hand-rolled Google authN #393, api-key via
  PrincipalKeyStore.verify #206, email-keyed admission on 48/49, role→capability a function,
  (principal,agent) seam), the read-only guard (one deny-by-default readOnlyHint middleware; #295
  EFFECT; coverage-as-checked-variable; composes with the RBAC PDP), the composition
  (FastMCP(auth=)+build_mcp_server http_client; BearerAuthMiddleware/AuthVerifier deleted;
  OriginValidationMiddleware outermost), config (no client_secret, ESC-2), posture derivation. Each
  wave: contract→adversary(SUFFICIENT after 1 revision)→build→cold-audit(GO)→commit. Gates at
  completion: wave-1 41/41, wave-2/3 90/0, loopback #295 7p/1s, typecheck 0, ruff clean, currency
  PASS (`bounds:[]` — all packet-39 pending bounds discharged). Fulfilled #395 (removed-behavior
  re-cut) + cold-audit R2 (config-validation). Design forks ruled by the Fable sidecar:
  provider-count (single-Google MVP, extendable), identity (email-now/seam-later), F4 role-gated →
  operator re-scoped to READ-ONLY (write → the 60–65 authz track), #393 hand-roll Google authN,
  ESC-2 no-client_secret (F2 structurally resolved). Residuals: config.keys orphaned
  (disposition→sidecar); 4 §8→§9 docstring citations (minor). Reports →
  `docs/plans/v2/receipts/2026-08-21-packet39-recut/`. Findings #393/#394/#395. ⚠ NO 39-alone
  deploy — packet 65 owns the joint 39+60–65 cutover + the live proof ladder (in-image conformance,
  `.well-known`/401, connector flow).

- 2026-08-22 · **PACKET 60 (Keep substrate) — CODE COMPLETE, COMMIT-ONLY.** The head of the 60–65
  authz/Keep track. Full pipeline (contract→adversary→build→cold-audit) Opus 4.8 end-to-end + a
  long-running Fable design sidecar; all coordination on lore_comms; operator offline (design forks
  ruled by the sidecar). Commits on `feat/surreal-unification`: `f0ebbf4` sidecar Forks A–F ·
  `efccdc8` W1 schema `keep`+`member_of{rank}` edges (OVERWRITE ENFORCED+UNIQUE(in,out), folded into
  generate_ddl) + `KeepStore` CRUD (atomic create_keep+auto-household, nameless-keep read-back,
  keeper-lockout, `_query` shared-retry) · `e019477` Q2b consumer-law amend (KeepStore wraps engine
  rejections as KeepStoreError, coverage-as-checked-variable) · `bd9032f` W2 `lore-adm` verbs
  (create-keep/add-household/remove-household/set-rank) · `db719a0` FR-3 (remove LOUD on a ghost keep
  + ∀-verb nonexistent-`--keep` invariant) · docs(60) `5b7180c`/`66ed1aa` (FR-2/FR-3 rulings) ·
  docs(49) `3077d23`/`52e5bb5` (stale-STUB comment cleanup on greened 49 code, #398) · report archives.
  Design: keeper an INDEXED `keep.keeper` FIELD LINK (**Fork A diverges from design §2.1's edge
  notation** — sidecar-ruled within delegated authority, semantically identical, countermand-able).
  Gates: 800 pytest / typecheck 0 / ruff clean; cold-audit GO (30-check live probe). The
  adversary/cold-audit loop caught + closed 5 real gaps (nameless-keep read-back · KeepStore engine
  leak · no-op set-rank · silent remove-on-ghost-keep · degenerate/test-env-fiction pins). Findings:
  #397 RESOLVED; #398 (stale-STUB invariant = Part 3, standalone, **before packet 61**), #400 (DRY
  lorerunes wrap-seam), #401 (test-env-fiction stderr) OPEN. Reports → `receipts/2026-08-22-packet60/`.
  ⚠ NO 60-alone deploy — packet 65 owns the joint 39+60–65 cutover (§8).

- 2026-08-23 · **PACKET 61 (Authorization PDP + audit) — DESIGN RULED + SPLIT (operator-ratified).**
  Fable sidecar `design-sidecar-61` ruled Forks A–L + FR-4 (`docs/design/2026-08-22-packet61-pdp-audit-rulings.md`);
  operator ratified all 5 flagged choices as recommended: (A) single-brain = a closed predicate IR
  with two total interpreters (`to_surql`+`matches`), `authorize()==authorize_filter().matches()`,
  equivalence pinned by a LIVE-STORE oracle differential test vs the real 3.2.4 engine (never a mock);
  (C) principal-private write/delete = exact `(principal,agent)`; (G) audit fires on load-bearing
  admin bypass; (SPLIT) 61→61a/61b (prices >0.30, sizing law); (FR-4/#402) keeper-on-principal-delete
  = refuse-while-keeping. Owner = two indexed fields `owner_principal`+`owner_agent`. Entry check
  surfaced a packet-60 escapee — **#402** (keep.keeper + member_of dangle on principal delete;
  `PrincipalStore.delete` cascade incomplete; the packet-49 exact-set pin RED_ORPHANED at HEAD) —
  sequenced as 61a-w1 (clears the gate). 61a = #402 → #400 wrap-extraction → #398/#399 invariant →
  `audit` store; 61b = probe → PDP core → resolver+coverage → dedicated security-auditor. COMMIT-ONLY
  (deploys with 39+60+62–65 at the packet-65 joint cutover). Findings #402 filed.

- 2026-08-23 · **PACKET 61a (Foundations + Audit) — CODE COMPLETE, COMMIT-ONLY.** All 4 sub-waves
  shipped on `feat/surreal-unification`, each full pipeline (contract→adversary→build→cold-audit GO
  →lead currency-verify) Opus 4.8 end-to-end + the Fable design sidecar; operator ratified the 5
  design forks + the §9 identity-at-write. **w1 #402** `66b795c` keeper-on-principal-delete =
  refuse-while-keeping + admin set-keeper/delete-keep (member_of auto-cascade probe-settled;
  RED_ORPHANED pin → GREEN). **w2 #400** `6ad460b` the shared engine-rejection wrap seam (two-layer
  `lorerunes.reclassify` + `_txn.wrap_store_rejection`, 9 pure-translate clones routed; property-
  derived spelling-agnostic shape-guard, allowlist-empty). **w3 #398/#399** `708c74e` the structural
  fold-coverage recurrence invariant (non-vacuous 28/28, INSTRUMENT-0, now live-gating the schema
  tree). **w4 audit store** `4f36782` append-only `audit` table + AuditStore (MRO-clean surface, ulid,
  born-wrapped, composable fragment; §9 identity-at-write actor_email/actor_agent_name survives
  principal-delete; dangle-tolerated cascade — the unifying role-based cascade principle). The reach
  law caught multiple hand-list defeats before shipping (Fork-I prose, coverage guard, standalone set,
  spelling variants, MRO surface); contract authors ground-truth-corrected 3 sidecar design premises.
  Findings #398/#399/#400/#402/#403 RESOLVED; #404–#412 ledgered (bounds/deferrals/#405 currency-gate
  `-n auto` flake). Reports → `receipts/2026-08-23-packet61/`. **61b (the PDP core) NEXT.** ⚠ NO
  deploy — packet 65 joint cutover.

- 2026-08-23 · **PACKET 61b (PDP engine + resolver + coverage) — CODE COMPLETE; PACKET 61 COMPLETE,
  COMMIT-ONLY.** The load-bearing authorization brain, on `feat/surreal-unification`, full pipeline
  (probe → contract→adversary SUFFICIENT→build→cold-audit GO→lead currency+receipt verify) Opus 4.8
  end-to-end + Fable design sidecar; a **dedicated SECURITY-AUDITOR** pass on the PDP core. **probe**
  `c6c6bc2` settled the owner-field index planner: IN→UnionIndexScan, composite leading-col-only, and
  **`IN` inside an `OR` is a TableScan trap — expand to per-value equalities** (#413; store ref §2).
  **w1 PDP core** `3b0894e` — one brain in `lorerunes.pdp`: a closed predicate **IR** with two TOTAL
  interpreters (`to_surql`+`matches`), so `authorize(s,a,r).allowed ≡ authorize_filter(s,a).matches(r)`
  is true BY CONSTRUCTION (`authorize = authorize_filter().matches()`), pinned by a **LIVE-STORE 3.2.4
  differential oracle**, never a mock; admin short-circuit a node OF the filter; audit-obligation flag;
  frozen stdlib `Subject`/`Resource`/`Decision`. The security-auditor caught **#416** (MEDIUM: an
  unparenthesized `ScopeInKeeps` disjunct — safe at top-level Or, a cross-principal leak under
  And-composition, invisible to the entry-point oracle) → every IR node now emits a self-contained
  parenthesized fragment + a live composition-safety pin (mutation M7) + SEC-F2/F3/F4/R4. **w2
  resolver+coverage** `af04b4a`/`650b37f`/`252c221` — `list_keeps_for_member` (plain-table leading-col
  IndexScan, bare ids, whole-method born-wrapped: it feeds the authz path) + `resolve_visible_keeps`
  (bare→`keep:<id>` frozenset via the SHARED `keep_scope`) + `partition_tools_by_population` (Fork H-a,
  DEFAULT-GOVERNED fail-closed, derived over the live registry; reds-until-reviewed growth pin — the
  reach law caught the sidecar's illustrative named-7, forcing `lore_verify`+`lore_index` classification
  → R1 ruled shared_read, seed = 9). Gates: 28 pins, full suite 8798p/0f solo, typecheck 0, ruff clean,
  currency PASS, mutation proofs RED-and-restored on independent instruments. Findings #413 + #416
  RESOLVED; #404–#419 ledgered (bounds/deferrals: #417 reach-law STOP-rule, Fork H-c verb-routing →
  pkt 63, #406 read-gap fold). Reports → `receipts/2026-08-23-packet61b-w1/` + `.../packet61b-w2/`.
  **With 61a: PACKET 61 (Authorization PDP + audit) is COMPLETE.** ⚠ NO deploy — packet 65 joint
  cutover (39+60+61+62–65). **62 (agent-identity binding / anti-spoofing) NEXT.**
- **2026-08-24 — PACKET 62 (agent-identity binding / anti-spoofing) CODE COMPLETE, COMMIT-ONLY.** 2
  waves, Opus 4.8 end-to-end + Fable design sidecar. Mechanism **(A)** app-layer per-agent CAPABILITY
  (operator-ruled: per-agent identity can't ride the shared MCP transport — HIGH-verified; real local
  principal via api-key overrode the sentinel). W1 owns-edge `agent.owner_principal` (`3c94ffd`); W2
  `agent.capability_hash` + verify-with-BINDING + `stamp_owner` + shared `lorerunes.parse_credential`
  (`66b5fa4`, 2 converging adversary rounds). **Dedicated security-auditor GO — 36 exploits 0-fail, §9
  anti-spoofing bar cleared.** Contract 64/0, blast 1388/0 (PrincipalKeyStore 0-regression), currency
  PASS. #423/#424 resolved (dnd probe deleted), #425 → 63/64. Reports `receipts/2026-08-24-packet62/`.
  ⚠ NO deploy — pkt 65 joint cutover. **63 (govern comms + memory) NEXT.**
- **2026-09-02 — PACKET 63a (governed substrate + memory retrofit) CODE-BUILT, NOT YET CLOSED —
  currency gate RED on 13 self-caused regressions.** The full 63a-a/ii/iii/iv/v pipeline landed
  (probe → contract → 2 adversary strikes → 2 cold-audit NO-GOs [F1 supersede-close bypass, F-A
  create-collision] → §10.9 design-class fix [owner-fold + F5 whole-tree reach] → 3 more adversary
  rounds [R3 exempt-frame, R4 shape-substring+verb-set] → operator OPTION-A [pin R4, root-fix→63b] →
  base build → closing cold-audit GO). HEAD `c794b21`; 30 reports archived `ab69bce` →
  `receipts/2026-09-02-packet63a/`. Delivered: `governed` substrate (resolve_subject/read_filter/
  guarded_write/StoreHandle) + `AppContext._resolve_subject` composition root + memory retrofit
  (subject=, GovernedDenied) + #425 additive + #439 owner-fold + F5 enforcement (whole-tree,
  both-ways, table-parametrised) + migrate-governed + keep.key. ⚠ **BUT the mandated solo currency
  gate came back RED (exit1) — 13 repo-wide invariant tests, ALL 63a-CAUSED** (triage #452: the
  scoped runs + all 4 cold-audits ran only test_*_63a*+mcp+61, NEVER the full repo suite; 63a is
  cross-cutting [new whole-tree parser, capability= param, schema folds, keep write, search render]
  and broke 7 standing derived invariants: test_tool_allowlist/#451, test_schema_rebuild-
  TestRebuildingNoticeSeam [was mis-called 'pre-existing'], test_ast_reach_helpers, test_link5_
  render_containment, test_engine_rejection_seam[keep], test_secret_typing, test_schema_fold_coverage).
  FIX WAVE = task `b5909325` (7 legitimate reconciliations — register each new element in its
  invariant's derived set; the tests ARE the RED pins). CLOSE GATE = the full currency gate green
  (0 RED_ORPHANED). ⚠⚠ **BLOCKED: subagent spawns stall at startup this session (3× consecutive —
  triage + 2 fix-wave respawns never registered/claimed); needs a FRESH SESSION to run the fix wave.**
  Everything is durably ledgered (finding #452 + task b5909325 carry the full worklist w/ file:line).
  Root-fixes → 63b: #449/task `571ef1a` (F5 statement-scope + verb-as-property), #436/#437/#441.
  LESSON (finding #452): a cross-cutting wave's cold-audit MUST run the full repo suite, not just
  the packet's own test_*_NN* files. **NEXT (fresh session): run task b5909325 → currency green →
  cold-audit → 63a closes; then 63b.**
- **2026-09-03 — PACKET 63a CLOSED (fix wave complete; currency green). COMMIT-ONLY (deploy = pkt
  65 joint cutover).** Ran task `b5909325`: the 13 RED_ORPHANED (finding #452, all 63a-caused) fixed
  as 7 one-concern commits on top of `9143df4`, each a REAL reconciliation of a NEW 63a element with
  a STANDING derived invariant (register/allowlist/partition/map + evidence), NEVER hack-to-green;
  RED-before/GREEN-after confirmed per fix. Commits: fix2 `f4a00fb` (test_schema_rebuild — the
  `_raise_if_empty_during_rebuild` gate now measures SUBSTANTIVE emptiness, `any(kind != NOTICE_KIND)`,
  not raw `if results:`; 63a-ii's unconditional `_MEMORY_WITHHELD_NOTICE` had defeated the raw check
  so an empty search mid-rebuild stopped raising) · fix1 `0c5fc23` (test_tool_allowlist —
  `_CAPABILITY_PARAM_DESCRIPTION` constant → `_capability_param_description(enabled)` via the shared
  `_guarded_description` seam so the `lore_comms` crossref DROPS when disabled; byte-exact at the full
  universe) · fix3 `278f524` (test_ast_reach_helpers — 5 evidence-backed non-adopter files allowlisted;
  ⚠ the brief named `_governed_contract.governed_table_raw_mutation_sites_in_tree` but the Layer-2
  detector globs only `test_*.py`, so that F5 parser is invisible to it; the real offenders were 3
  test-tree meta-scanners + 1 single-package scan + 1 `re.compile` false-positive) · fix4 `5086235`
  (test_link5_render_containment — `capability` → `_NOT_SERVED_OUT`; the credential value is traced
  DOWN-not-OUT and reaches no served render/error) · fix5 `0ed3215` (test_engine_rejection_seam —
  `get_or_create_keyed`, a 63a routed KeepStore write path, added to `_KEEP_INVOCATIONS`) · fix6
  `2f2caae` (test_secret_typing — two 63a spike-store probe `SecretStr("spikeroot")` mints
  adjudicated as credential origins; ⚠ #452's "capability mint" triage wording was imprecise — the
  real offenders were `probe_dnd_defense_objects.py`+`probe_read_filter_63.py`) · fix7 `bb31a86`
  (test_schema_fold_coverage — `_schema_fold_guard` coverage broadened from direct-call to a ROOTED
  transitive closure so a nested sub-slice folded via a folded parent is covered; +2 discrimination
  pins; ⚠ there is NO hand-list "map" to register into — the guard is AST-derived, so the brief's
  "register in the map" model was wrong; production schema UNCHANGED). **Fixes 4–7 built by 4 parallel
  Opus-4.8 builder subagents (lore_comms-coordinated); fixes 1–3 lead-direct.** GATES: currency gate
  **PASS — 0 RED_ORPHANED, true exit 0**, ruff + `typecheck.sh` GREEN, verified by TWO independent
  full-repo runs (lead + a cold-auditor's own re-run). ⚠ Both fresh-context cold-auditor spawns
  STALLED (the prior session's in-process-subagent-stall issue reproduced — the first ran its currency
  re-run to green then went silent ~15min, the second wrote nothing) → per the brief's fallback, a
  LEAD SELF-AUDIT with EMPIRICAL construction/mutation proofs stands in: fix2 revert→A8a RED (gate is
  load-bearing), fix7 unroot→`test_a_nested_slice_whose_parent_is_unfolded_is_flagged` RED (closure is
  genuinely rooted), fix1 byte-exact vs `git show 9143df4`, fix4 security-grep (no raw-credential
  render). VERDICT GO, no suppression/weakening/false-clear. Reports (4 builder + lead audit) →
  `receipts/2026-09-03-packet63a/`. RESIDUALS (non-blocking): #452 triage wording imprecise on fix3
  & fix6 (fixed to ground truth); DRY — spike-probe connect boilerplate cloned 8+× (a shared
