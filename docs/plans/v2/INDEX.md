...
| 04b-1 | **task DAG + the sender guard** — `blocks` edge (`ENFORCED` from birth, in `_task_statements`), `create_task` → transactional, mirror from `blocked_by` at both write paths, the shared blocker-existence pre-check (generalised, NOT cloned), edge≡`blocked_by` + acyclicity + the `+collect` transitive read, the relation-edge mutation proof widened to all FIVE edges, **#247** sender guard at `MessageLedger.send`, **#253** (`query_tasks`' unbounded whole-table read — operator-ruled in 2026-07-28), 04a residuals R-1/R-2 | PKT-28 C2c | C | 0.30 | 04a | open — **TEST-ONLY + schema, no deploy** (04b-2 deploys). ⚠ **Writable set WIDENED into `server.py` by two NAMED exceptions** (rulings R5 `limit` push-down + R6 cycle-policy unification); `server.py` is otherwise 04b-2's. Contract graded **INSUFFICIENT** at `f3c8b24` (3 wrong builds survived 80/80) → contract-fix wave. |
| 04b-2 | **comms surface** — blocked-chain/critical-path render, fleet unread + unacked-directive columns (`acked_at IS NONE`, `grade='directive'`), `_comms_footer` + optional `agent`/`session` on the three ledger tools + `_INSTRUCTIONS`, **#219** (4 prose sites), 04a residuals R-5/R-12 | PKT-28 C2c | C | 0.25 | 04b-1 | open — **DEPLOYS** (carries 04b-1). |
