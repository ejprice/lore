...
| 62 | **Agent-identity binding (anti-spoofing)** — server-bound, short-lived `(principal, agent)` credentials + the two anti-injection invariants (identity/owner NEVER from a tool argument; scope-on-write PDP-validated). The §3.3/§10-O seam; dedicated SECURITY-AUDITOR pass (it IS the anti-spoofing surface). Design §3 | — | **D** | 0.25–0.30 | 39, 60 | **PLANNED.** Contains prompt-injection blast-radius to the authenticated (principal, agent). |
