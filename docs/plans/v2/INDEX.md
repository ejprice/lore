...
| 61 | **Authorization PDP + audit** — the in-process `authorize`/`authorize_filter` engine (`lorerunes`/`loremaster`): single-brain PER ACTION, the 3-way write rule (private=owner · keep=household · server=owner+admin), the admin short-circuit, the visible-Keeps resolver; + an append-only, ADMIN-EXEMPT `audit` store. Design §5/§6/§9 | — | **D** | 0.30 | 60 | **PLANNED.** The one policy brain everything routes through; coverage-as-checked-variable; mutation-proven read/write equivalence. |
