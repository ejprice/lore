...
| 63 | **Governed-tool retrofit: comms + memory** — add `owner`+`scope` (server-stamped, never a caller arg), route every read through `authorize_filter` and every write/delete through `authorize`; migrate existing free-form-owned rows (dirty-store, #107/#131 class). Design §4/§7 | — | **D** | 0.30 →split if ≥ | 60, 61, 62 | **PLANNED.** |
