...
| 47a | **twelfth seam — BUILD** (implement the ruled 47 design: contract → contract-adversary → build → cold audit; store-touching — typed entity records + RELATE edges + fake-domain DDL in the per-file txn) | — | D | 0.32–0.38 | 47 | open — **NEXT** (fresh session). Mutation-prove: transactionality (entity fail ⇒ manifest INDEXED row rolled back, +control) · dirty-store ENFORCED migration (OVERWRITE lands ENFORCED, #107 shape) · cross-book newer-wins (full-sweep re-points at the newer-edition node) · resolve-or-drop (one unresolvable endpoint ≠ whole-scope abort) · the F5 in-`delete_by_tier` purge sink. Gates zero-new-delta vs #333. 52a's node-fragment ingest depends on this. |
