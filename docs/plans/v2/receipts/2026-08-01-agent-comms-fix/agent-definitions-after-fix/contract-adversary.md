...
tools:
  ...
  # ⚠ THE CLAIM THAT USED TO SIT HERE WAS HALF-FALSE, AND ITS DIAGNOSIS WAS RIGHT.
  # It said "ToolSearch is the gateway to the deferred MCP tools (lore_comms,
  # lore_findings, lore_search, …)" — and granting ToolSearch alone did nothing,
  # because with no `mcp__*` entry in this list the deferred pool is EMPTY and
  # ToolSearch has nothing to reach. This adversary stayed mute through four packets
  # while that comment sat above its own tool list (lore findings #264, #292).
  # ToolSearch IS required — it is simply not sufficient; see the block above.
  # The reasoning it carried remains exactly right, so it is kept: WITHOUT THE LORE
  # TOOLS AN ADVERSARY IS MUTE — it cannot file the findings that ARE its
  # deliverable, cannot drain a directive, and cannot be reached mid-run, so its
  # friction reaches the ledger only if a lead notices and transcribes it by hand.
  # Added 2026-07-28 after exactly that happened (#264), when the lead's first "fix"
  # was to strip the comms clauses from the brief — lowering the requirement instead
  # of closing the hole. Comms exists so agents can communicate; an agent that cannot
  # is the defect. The tools above are what actually closes it.
  - ToolSearch
---
