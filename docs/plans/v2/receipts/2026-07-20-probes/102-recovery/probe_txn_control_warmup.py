...
async def worker(worker_id: int, database: str, txn_count: int) -> dict[str, Any]:
    ...
    await connection.query("RETURN 1;")  # WARM-UP query before the real loop
...
async def main() -> None:
    ...
    print(f"=== workers: {WORKER_COUNT}, txns/worker: {TXNS_PER_WORKER} (NO sequence::nextval, WARMED-UP connection) ===")
