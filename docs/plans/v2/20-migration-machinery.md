# 20 — Fleet-migration machinery (Shape D, ledger #11) (formerly PKT-12)
size ~0.30 wu →split at kickoff (sizing law) · wave M (re-waved 2026-07-14) · depends: **operator answers to §8 Q1–Q4 at kickoff** (INDEX pool item 2 — the one ruling on the v1.0 critical path); packet 19; packet 18 before this finalizes
...
`<slug>.memory.db` write-through ledger. packet 22 executes; this packet only builds.

## Scope IN (the §7 numbered machinery)
- **N1 — RESHAPED by the singular-store ruling (2026-07-11, DESIGN-LAW §14):** v2 boot
  replay no longer exists (packet 18 retires it) — build the skip-and-log replay + the
...
fixture (never DI) with parity receipts; packet 22 unblocked; INDEX row + Log.
