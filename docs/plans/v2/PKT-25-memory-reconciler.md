# PKT-25 — Memory reconciler + maintenance engine (operator-directed 2026-07-11)
size ~0.35 wu · wave F · depends: v1.0 (deterministic tier); PKT-14 loresage (LLM tier)
...
## Scope IN — deterministic tier (no LLM; can ship before PKT-14)
...
engine must not break); PKT-19's expiry sweep state (this engine and PKT-19 share the
