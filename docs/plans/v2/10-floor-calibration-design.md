...
3. Drift measurement + thresholds (the current corpus-fingerprint trigger disarms only;
   ...
   Consider packet 35's planned live per-hit cosine distributions as a drift-evidence
   input (continuous signal vs periodic survey) — design for it even if packet 35 lands
   later.
...
operator ruling recorded in the doc; INDEX row flipped; packet 11 unblocked.
