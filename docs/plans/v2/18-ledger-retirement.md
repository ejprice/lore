# 18 — SQLite ledger retirement (singular-store ruling, 2026-07-11) (formerly PKT-24)
size ~0.20 wu · wave M, FIRST (re-waved 2026-07-14; MUST land before packet 20 finalizes and packet 22 ships) · depends: none
...
- **Preserve the reader for migration only**: packet 20's N3 driver imports v0.3
...
- **Backup posture, per mode**: document REQUIRED store-level backups — single-node:
  ...
  packet 19 — build it here, packet 19 inherits it).
- **Docs truth**: README/positioning may now claim the singular store outright
  (coordinates with packet 09's positioning pass).
