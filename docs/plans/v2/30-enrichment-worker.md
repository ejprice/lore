# 30 — Enrichment worker: llm_summary → BM25 (formerly PKT-15)
size ~0.30 wu · wave F · depends: packet 29
...
- Undocumented findings seeding (a summary can seed a kind=undocumented finding so the
  fix lands in code) — the finding side only; detectors proper are packet 31.
- Enrichment traces stamped with token/model/cost (packet 29's hooks).

## Scope OUT (recorded deferrals — do not build)
- Vector-side re-embedding with summaries: v1.2+ (schema-fingerprint implications).
- Docstring as a separate weighted BM25 field: packet 28.

## Entry check
packet 29 landed; a real undocumented module identified on the lore corpus as the smoke
target.
