...
**Amended same-session (2026-07-31), two lead pings:** R12 (a further operator override —
the allowlist substrate moves out of `lore.yaml`, §3-R12) and §7 (the annotations derivation
...
> - "a principal removed from the roster file is still admitted on a later request" —
>   BLOCKER (admission is evaluated per verification against the live roster; the token
>   cache caches GOOGLE's verdict, never admission — see R12).
...
and revocation must never require a redeploy — especially while **#165 is open** and a
recreate re-mounts `/source` and re-breaks boot, making "recreate to revoke" actively
dangerous.
...
- **The roster lives outside the repo and outside the image**, bind-mounted READ-ONLY into
  lore-lore (host-side sibling of the existing secrets-mount idiom; exact host path chosen
  at build and recorded in the deploy docs). Plain newline-delimited email addresses; `#`
...
- **Freshness: one `stat` per verification; a full re-read only when mtime changed.** The
  override's floor is "at most one stat per cache-miss, never one read per request"; this
  design deliberately EXCEEDS it (stat on every verification, cache hits included — a stat
  is microseconds) because rider (a) below demands that the *next verification* sees a
  roster edit, and a miss-only stat would let a positive-cached token outlive revocation
  by the cache TTL. Decided under the override's no-prompt authority; if the operator
  prefers the weaker floor, the stat moves to miss-only and revocation latency becomes
  ≤ the positive-cache TTL — that is the one named alternative, not a re-open.
- **Admission is evaluated on EVERY verification against the live in-memory roster** — the
  token cache caches Google's verdict (token → subject/email/expiry), never admission.
  **Rider:** a cached-token principal is denied on the FIRST verification after the roster
  drops them (fixture drives a cache hit).
...
- **Must CHANGE from the port** (each confirmed against the odoo-code source this session):
  ...
  - **Real scope check:** require `"email"` in tokeninfo's returned `scope` set (comparing
    our own hardcoded request scopes to themselves — odoo-code's shape — is a tautology).
  - **Admission checked on EVERY verification, cache hits included** (odoo-code evaluates
    its allowlist only on the miss path, so a cached token outlives revocation by the
    cache TTL). The cache caches Google's verdict; the roster predicate runs per call
    against the live R12 roster. **Rider:** the cached-token-revocation pin in R12.
  - Injected `httpx.AsyncClient` (constructor param; tests use `httpx.MockTransport`),
...
with a fully-tested pass-through default. Derived at tool dispatch from
...
- **Granularity is the TOOL, deliberately:** `lore_index` (can reconcile), `lore_tasks`,
  ...
  guard.
- **The known-wrong hand-list (defect CONFIRMED in-flight, independently twice):**
  `test_mcp_server.py`'s `_MUTATING_TOOLS = {lore_remember, lore_index, lore_findings,
  lore_comms}` omits `lore_claim_task`/`lore_tasks`, which production annotates
  `readOnlyHint=False` via `_TASK_TOOL_ANNOTATIONS` — found by this design at source and
  independently by the lead reading the six annotation constants and their call sites
  (and verifying `FastMCP.tool()` accepts `annotations=`). A hand-list beside derived
  metadata, already drifted: the repo's own ONE-IMPLEMENTATION failure living inside a
  gate. **Fix (lead-directed): the hand-list is DELETED, not corrected** — the
...
1. **Allowlist predicate + roster substrate (R12)** — case-fold admission ·
   ...
   principal denied on the next request (cache-hit fixture) · mtime-check no-op mutation
   ⇒ RED · dot/plus aliasing NOT stripped (documented bound, pinned as behavior).
...
## 10. Kill switches (rewritten under R2 — ordered by speed, all lore-scoped first)

1. `systemctl --user stop lore-caddy` (this box, no root) — public reachability dies at
   once; local loopback service continues.
2. `podman stop lore-lore` — total stop.
3. Remove one principal: delete their line from the roster file (R12) — takes effect on
   the NEXT verification; no recreate, no rebuild, no restart. Emptying or removing the
   file denies EVERY principal (fail closed) — the per-principal switch and the
   all-principals switch are the same file, and this is now the FASTEST per-principal
   switch in the list. (The investigation's recreate-based revocation is superseded by
   the R12 operator override.)
...
- Blanking or removing the roster file denies ALL principals at runtime (R12) — effective
  on the next verification, and unlike the inherited F4 shape it can never fail open; at
  the next boot it is a refuse-to-boot. Listed here for completeness — switch 3 covers it.
...
- **Google tokeninfo response fields** (`exp` vs `expires_in`, `sub` presence) — settled
  at build against the vendor doc + a live probe (READ-THE-DOCS-THEN-VERIFY), not assumed
  here.
