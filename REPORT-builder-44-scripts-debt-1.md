...
5. **"Your §D2 question was answered hours ago and the answer was lost in transit… you were never told."** — **Not so, and the correction matters because it is a receipt FOR the fix.** The grant reached me exactly as intended: I pulled it from `lore_comms brief_get name=pkt44` as ruling **R7**, and implemented it at `96816cc` **before** the re-delivery arrived. **The durable channel worked on its first live use.** What was stale was the lead's view of my state, not my knowledge of the grant. Recorded because the opposite conclusion — "the durable channel also failed" — would be the wrong lesson to draw from a packet that exists partly to install that channel.

### I.1 Dogfood observation — `lore_comms` refused my over-cap message TWICE, correctly

My status reply came in at 2780 chars, then 2035, against a 2000-char cap. Both were **REFUSED with the cap, the actual length, and the reason** (*"messages carry POINTERS: put the content in a report or finding and reference it in refs"*) — **never truncated**. That is the trust doctrine behaving exactly as specified: a silent truncation would have delivered a message that *looked* complete while dropping the two open questions at the end, and I would have had no way to know. **Loud failure over silent short-serving, on a surface where the consumer is an agent that cannot tell the difference.** No finding filed — this is the design working, and it is worth recording as a positive receipt rather than friction. It also forced the message into the shape the tool intends (pointers + `refs`), which is the better artifact: the content lives in §H and #288 where it is durable and citable, not in an inbox.
