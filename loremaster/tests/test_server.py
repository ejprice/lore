...
class _NoConfigExtension(Extension):
    """A bare extension (no ``config_model``, no chunkers) for MANUAL-register units.

    Discovery cannot exercise ``register_extension``'s chaining return (it calls the
    method internally and discards the value), so the chaining unit registers this
    double manually against an empty-slice config — where seam-7 validation no-ops.
    """

    @property
    def name(self) -> str:
        return "no_config_ext"

@pytest.fixture()
def config_path(tmp_path: Path) -> Path:
    """Write a valid tiered ``lore.yaml`` for a BARE server (empty ``extensions``).

    ``minimal_config()`` now defaults to ``extensions={}`` (packet 46), so
    ``from_config(config_path)`` composes a plain RAG under live discovery. Tests
    that want the ``fake`` extension WIRED use the ``fake_discovered`` fixture
    (discovery-driven); tests of ``register_extension``'s own guards register a
    no-``config_model`` double manually against this bare config.
    """
...
@pytest.fixture()
def fake_discovered(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """A ``lore.yaml`` whose ``fake`` extension is composed by LIVE DISCOVERY.

    Injects ``{"fake": FakeExtension}`` into the registry and writes a config with
    the matching ``{"fake": {"flavour": "vanilla"}}`` slice, so
    ``LoreServer.from_config`` composes ``fake`` the production way — no manual
    ``register_extension``. The discovery-driven replacement for the old
    ``.register_extension(FakeExtension())`` idiom (packet 46 reconciliation).
    """
...
class TestRegisterExtensionWiring:
    """Live discovery composes each seam into the server surface.

    Packet 46: a config NAMING an extension (registry injected via
    ``fake_discovered``) has it instantiated + registered at construction — the
    production composition path. These tests exercise THAT path, not a manual
    ``register_extension`` (whose own guards are unit-tested below). Discovery calls
    the same ``register_extension`` internally with the same instance, so the seam
    wiring is exercised transitively.
    """

    def test_discovery_composes_the_configured_extension(
        self, fake_discovered: Path
    ) -> None:
        # The ``fake`` extension named in the config is discovered + registered at
        # construction — no manual register call (the production path).
        server = LoreServer.from_config(fake_discovered)
        assert [e.name for e in server.extensions] == ["fake"]

    def test_register_returns_self_for_chaining(self) -> None:
        # MANUAL register of a no-``config_model`` double against an empty-slice
        # config: the SOLE way to observe ``register_extension``'s chaining return
        # (discovery calls it internally and discards the value). Preserves the
        # returns-self/chaining behaviour the old discovery+chaining test pinned
        # (DUAL — removed-behaviour inventory).
        server = LoreServer(minimal_config(extensions={}))
        ext = _NoConfigExtension()
        assert server.register_extension(ext) is server
        assert [e.name for e in server.extensions] == ["no_config_ext"]

    def test_seam1_fake_chunker_becomes_dispatchable_on_real_makefile(
        self, fake_discovered: Path, tmp_path: Path
    ) -> None:
        # Seam 1 over a REAL file: a Makefile (basename-keyed, which the suffix
        # registry cannot route) becomes dispatchable once discovery composes the
        # extension chunker, and produces chunks driven by the real file content.
    ...
    def test_seam2_xml_profile_reaches_constructed_chunker(
        ...
        # Seam 2 over a REAL file: with the fake threshold profile composed via
        # discovery, the constructed XmlChunker forces each <threshold> into its own
        # chunk, so a file that collapses to ONE chunk bare now yields MANY
        # profile-claimed chunks. This proves the profile reached the built chunker.
        if not _REAL_XML.exists():
            pytest.skip(f"{_REAL_XML} not present on this host")
    ...
    def test_seam8_payload_indexes_are_collected(self, fake_discovered: Path) -> None:
    ...
    def test_seam10_source_providers_are_collected(self, fake_discovered: Path) -> None:

        server = LoreServer.from_config(fake_discovered)
        assert [p.tier for p in server.source_providers] == ["vendor"]

    def test_seam5_format_result_override(self, fake_discovered: Path) -> None:

        server = LoreServer.from_config(fake_discovered)
        ctx = server.extension_context(store=object())
        # The composed extension's format wins over the base default.
        assert server.format_result(_candidate("a", 0.9), ctx) == "FAKE: a"

    def test_seam6_chunk_key_override_carries_version(self, fake_discovered: Path) -> None:
    ...
    def test_seam11_classify_detail_override(self, fake_discovered: Path) -> None:
    ...
    def test_seam4_search_hooks_transform_candidates(self, fake_discovered: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_seam9_lifespan_hooks_awaited(self, fake_discovered: Path) -> None:
...
class TestConfigModelValidation:
    """The extension's ``config_model`` validates its ``extensions[name]`` slice.

    Valid + bad slices flow through DISCOVERY (the production path, where validation
    fires at boot inside discovery's internal ``register_extension``); the
    missing-slice branch flows through a MANUAL register, because discovery can
    never reach it (see ``test_missing_slice_for_extension_with_a_model_fails_loud``).
    """

    def test_valid_slice_passes(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        # Discovery composes ``fake`` and validates its slice at construction.
        register_in_discovery(monkeypatch, {"fake": FakeExtension})
    ...
    def test_bad_slice_fails_loud(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        # The ``fake`` config model forbids extras; a typo'd key in the slice must
        # raise where production raises it — at BOOT, inside discovery's internal
        # ``register_extension`` — not be silently dropped.
        register_in_discovery(monkeypatch, {"fake": FakeExtension})
        config = minimal_config(extensions={"fake": {"flavor": "misspelled"}})
        path = tmp_path / "lore.yaml"
        path.write_text(yaml.safe_dump(config.model_dump(mode="json")), encoding="utf-8")
        with pytest.raises(ValidationError):
            LoreServer.from_config(path)

    def test_missing_slice_for_extension_with_a_model_fails_loud(
        self, tmp_path: Path
    ) -> None:
        # ROLE (packet 46, KEEP MANUAL): this is the SOLE coverage of the
        # missing-slice fail-loud branch of ``_validate_extension_config``, which is
        # UNREACHABLE via discovery (discovery always supplies the iterated key's
        # slice). It MUST stay a MANUAL ``register_extension`` against a config that
        # LACKS the extension's slice — do NOT "helpfully" fold it into discovery,
        # which would silently orphan the branch (a green suite over dead code).
...
class TestNit1RegisterGuard:
    ...
    These unit-test ``register_extension``'s OWN chunker-overlap guard, so they
    register MANUALLY. Each shadow extension subclasses the BARE ``Extension`` (no
    ``config_model``, so seam-7 slice validation no-ops against the empty-slice
    ``config_path``) and overrides only ``chunkers`` — isolating the nit-1 guard
    from slice validation. (Packet 46: rebased off ``FakeExtension``, whose required
    ``config_model`` would otherwise raise before the chunker guard is reached.)
    """
    ...
    def test_overlapping_default_suffix_claim_raises(self, config_path: Path) -> None:
        ...
        class _ShadowExtension(Extension):
            @property
            def name(self) -> str:
                return "py_shadow"
    ...
    def test_filename_keyed_chunker_without_a_suffix_is_allowed(
        ...
        # WITHOUT blocking. A bare extension exposing ONLY that chunker isolates the
        # guard's allow-path from any slice validation.

        class _MakefileExtension(Extension):
            @property
            def name(self) -> str:
                return "makefile_ext"
        ...
        server = LoreServer.from_config(config_path).register_extension(
            _MakefileExtension()
        )
        assert [e.name for e in server.extensions] == ["makefile_ext"]
    ...
    def test_greedy_handles_without_declared_suffix_is_refused(self, config_path: Path) -> None:
        ...
        class _GreedyExtension(Extension):
            @property
            def name(self) -> str:
                return "greedy_ext"
