...
class FakeMessageLedger:
    ...
    # NOTE (packet 05a-ii): the ledger owns NO await method — R-1 (lead-ratified)
    # hosts the wait-machine in a STANDALONE ``InboxAwaiter`` constructed with an
    # injected ``connect`` factory + this ledger's ``drain`` seam. The awaiter CALLS
    # ``drain(peek=True)``; the ledger stays the data-access authority.
