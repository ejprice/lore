...
# WIDTH SPACE, U+FEFF ZERO WIDTH NO-BREAK SPACE/BOM, which can hide characters
# inside it); and U+2028 LINE SEPARATOR / U+2029 PARAGRAPH SEPARATOR (real
# line breaks to a bidi-aware terminal or browser-rendered UI even though
# neither is ``\n``, so left uncollapsed they could fracture a single
# rendered line into a fake second one). A run of any of these collapses to a
# single space so a hostile identity/path/memory text stays one logical,
# visually-honest line and cannot smuggle terminal-framing, a fake second
# citation, or a hidden/reordered payload into a rendered field.
...
def _sanitise_line(text: str) -> str:
    ...
    override/isolate/mark, zero-width, and line/paragraph separator characters)
